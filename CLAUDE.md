...
Whenever you make a change to the codebase (new feature, bug fix, refactor, etc.), you must leave a record of that change under `docs/superpowers/`, in addition to updating the current-state specs under `docs/spec/`. `docs/spec/` describes what the system looks like *now*; `docs/superpowers/` records *why and how* it got there.
...
├── specs/
│   └── YYYY-MM-DD-{change-name}-design.md
└── demos/
    └── {change-name}.html (or other demo asset, optional)
```

#### Naming Convention

- `{change-name}` is a short kebab-case slug describing the change (e.g. `subtitle-mode`, `bing-translator-integration`).
- `YYYY-MM-DD` is the date the plan/design was authored, not the date it merges.
- A single change typically produces a matching pair: `plans/YYYY-MM-DD-{change-name}.md` and `specs/YYYY-MM-DD-{change-name}-design.md`. Use the same date and slug for both so they can be paired by filename.

#### What Each File Must Contain

- **`specs/YYYY-MM-DD-{change-name}-design.md`** (design doc, write this first): Context/motivation, Non-Goals, user-visible behavior, architecture, and any UI layout details needed for reproducibility.
- **`plans/YYYY-MM-DD-{change-name}.md`** (implementation plan): Goal, architecture summary, and a File Structure table (`Created` / `Modified` / `Reused without modification`) listing every file touched and why. Reference the paired design doc at the top.
- **`demos/`** (optional): static HTML or other artifacts used to preview/demo the change.

#### Rules

1. Every non-trivial change must have at least a plan entry under `docs/superpowers/plans/`; add a paired design doc under `docs/superpowers/specs/` when the change affects user-visible behavior or architecture.
2. Never delete or rewrite past entries in `docs/superpowers/` — they are an immutable history log. If a later change supersedes an earlier one, add a new dated entry instead of editing the old one.
3. After finishing a change, also update the relevant files under `docs/spec/` so that directory always reflects the current state of the system.
